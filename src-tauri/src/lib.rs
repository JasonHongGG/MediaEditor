...
fn build_filter_graph(request: &TimelineExportRequest) -> Result<(Vec<String>, String, Option<String>), String> {
    ...
    let (width, height) = if is_video_output {
        scaled_dimensions_for_quality(
            ...
            request.video_quality.as_deref(),
        )
    } else {
        (0, 0)
    };
    ...
    let used_source_ids: HashSet<&str> = session.clips.iter().map(|clip| clip.asset_id.as_str()).collect();
    ...
    let video_output_label = if is_video_output {
        ...
        let mut video_clips: Vec<&TimelineClipPayload> = session
            .clips
            .iter()
            .filter(|clip| {
                source_map
    ...
    let mut audio_clips: Vec<&TimelineClipPayload> = session
        .clips
        .iter()
        .filter(|clip| {
            !clip.muted
...
fn codec_args_for_format(format: &str, audio_bitrate_kbps: Option<u32>) -> Vec<String> {
    let bitrate = audio_bitrate_kbps.unwrap_or(320).clamp(96, 320);
